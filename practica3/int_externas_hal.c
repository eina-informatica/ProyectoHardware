...
// Verificar si la interrupción externa 2 sigue pulsada
...
// Habilitar las interrupciones de la interrupción externa 2
void eint2_habilitar_interrupciones(void) {
    if((IOPIN & (1<<15))!=0){
		// Limpiar la flag de la interrupción externa 2
    EXTINT = EXTINT | 4;
		eint2_nueva_pulsacion = 0;
    VICIntEnable = VICIntEnable | 0x00010000;
	  PINSEL0 = PINSEL0 | 0x80000000;
...
// Función de interrupción externa 2
void eint2_ISR(void) __irq {
	if(eint2_nueva_pulsacion == 0){
    // Llamar a una función (cola_encolar_eventos) para encolar eventos
    callback_fuc(Eint2, 2);
